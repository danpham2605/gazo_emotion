...
def get_list_wav_and_emotion(path_audio,path_split_audio,model_bert,type_model="base"):
    ...
    duration_audio= audio_pydub.duration_seconds
    print("result[segments] ", len(result["segments"]),duration_audio,result["segments"]) 
    i=0
    end_audio=0
    while(i<len(result["segments"])):
        value=result["segments"][i]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--scale_image', default=0.9)
    ...
    parser.add_argument('--path_input_audio', default="/media/anlab/DATA/gazotuber/End2End_gazo/Input_to_Gazo2.wav") #input audio
